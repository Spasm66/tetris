...
game game_new_ext(uint nb_rows, uint nb_cols, square* squares, uint* nb_tents_row,
                  uint* nb_tents_col, bool wrapping)
{
  ...
  for (uint j = 0; j < g->nb_cols; j++) g->nb_tents_col[j] = nb_tents_col[j];

  // set expected nb tents in each row
  for (uint i = 0; i < g->nb_rows; i++) g->nb_tents_row[i] = nb_tents_row[i];
...
{
  if (wrapping) assert(nb_rows >= 3 && nb_cols >= 3);
